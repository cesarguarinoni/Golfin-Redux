...
async function exportHole(courseId, holeNumber, courseJson) {
  ...
  // --- Create flat variant ---
  createFlatExport(exportDir, path.join(ROOT, 'output', courseId, 'export', `hole-${nn}-flat`));
...
function createFlatExport(normalExportDir, flatExportDir) {
  ...
  // 2. Generate flat heightmap (average elevation of normal)
  const RES = 2049;
  ...
  // 3. Update manifest max_elevation_m to minimal range
  const manifestPath = path.join(flatExportDir, 'hole-manifest.json');
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf-8'));
  manifest.terrain.max_elevation_m = 1;
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf-8');

  console.log(`  Flat variant: avg=${avgValue} → ${path.basename(flatExportDir)}`);
}
