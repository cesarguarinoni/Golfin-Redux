...
export function sampleDem5a(dem5aTiles, lat, lon) {
  for (const tile of dem5aTiles) {
    ...
    // Bilinear sample — much smoother than nearest-neighbour.
    const fx = (lon - bounds.west) / (bounds.east - bounds.west) * (TILE_SIZE - 1);
    const fy = (bounds.north - lat) / (bounds.north - bounds.south) * (TILE_SIZE - 1);
    ...
    const tx = fx - x0;
    const ty = fy - y0;
    const v00 = grid[y0 * TILE_SIZE + x0];
    const v10 = grid[y0 * TILE_SIZE + x1];
    const v01 = grid[y1 * TILE_SIZE + x0];
    const v11 = grid[y1 * TILE_SIZE + x1];
    // Fall back to nearest if any corner is NaN
