...
export const DICT = {
  ...
  // ---- Loans ops panel (loans_ops §3.3) -------------------------------------
  "nav.loans": { en: "Loans", ja: "貸し出し" },
  "loans.title": { en: "Loans", ja: "貸し出し" },
  "loans.subtitle": {
    en: "Server truth · lazy expiry — an offer past its clock shows as offered until somebody reads it. Every action here needs a note and lands in the audit log.",
    ja: "サーバーの実データです。期限切れは遅延処理のため、期限を過ぎたオファーも誰かが読み込むまでは「オファー中」と表示されます。ここでの操作はすべてメモが必須で、監査ログに記録されます。",
  },
  "loans.log.title": { en: "Loan log", ja: "貸し出しログ" },
  "loans.empty": { en: "No loans match these filters.", ja: "条件に一致する貸し出しはありません。" },
  "loans.filter.status": { en: "Status", ja: "ステータス" },
  "loans.filter.kind": { en: "Kind", ja: "種類" },
  "loans.filter.q": { en: "Player / loan id / ref", ja: "プレイヤー・ID・ref" },
  "loans.filter.qPlaceholder": { en: "display name, uuid or ref_id", ja: "表示名・UUID・ref_id" },
  "loans.filter.from": { en: "From", ja: "開始日" },
  "loans.filter.to": { en: "To", ja: "終了日" },
  "loans.filter.apply": { en: "Apply", ja: "適用" },
  "loans.filter.clear": { en: "Clear", ja: "クリア" },
  "loans.filter.all": { en: "All", ja: "すべて" },
  "loans.export": { en: "Export CSV", ja: "CSV 出力" },
  "loans.exportHint": {
    en: "Exports the filtered set, one row per loan, up to 500.",
    ja: "絞り込んだ結果を貸し出し 1 件につき 1 行で出力します（最大 500 件）。",
  },
  "loans.page": { en: "Page {n}", ja: "{n} ページ" },
  "loans.prev": { en: "← Newer", ja: "← 新しい方へ" },
  "loans.next": { en: "Older →", ja: "古い方へ →" },

  "loans.status.offered": { en: "OFFERED", ja: "オファー中" },
  "loans.status.active": { en: "ACTIVE", ja: "貸出中" },
  "loans.status.returned": { en: "RETURNED", ja: "返却済" },
  "loans.status.expired": { en: "EXPIRED", ja: "期限満了" },
  "loans.status.declined": { en: "DECLINED", ja: "辞退" },
  "loans.status.rescinded": { en: "RESCINDED", ja: "取り消し" },
  "loans.status.offer_expired": { en: "OFFER EXPIRED", ja: "オファー失効" },
  "loans.kind.character": { en: "character", ja: "キャラクター" },
  "loans.kind.club": { en: "club", ja: "クラブ" },

  "loans.col.when": { en: "Offered", ja: "オファー日時" },
  "loans.col.parties": { en: "Lender → borrower", ja: "貸し手 → 借り手" },
  "loans.col.days": { en: "{n} d", ja: "{n} 日" },
  "loans.col.level": { en: "Lv {from} → {to}", ja: "Lv {from} → {to}" },
  "loans.col.rp": { en: "RP {lender} / {borrower}", ja: "RP {lender} / {borrower}" },
  "loans.col.rpHint": {
    en: "RP paid to the lender / earned by the borrower on rounds played with this asset (running totals on the row).",
    ja: "このアセットでプレイしたラウンドで貸し手に支払われた RP / 借り手が獲得した RP（行の累計）。",
  },
  "loans.openUser": { en: "Open in Users", ja: "ユーザー画面で開く" },
  "loans.rel.in": { en: "in {n}{unit}", ja: "あと {n}{unit}" },
  "loans.rel.ago": { en: "{n}{unit} ago", ja: "{n}{unit}前" },
  "loans.unit.m": { en: "m", ja: "分" },
  "loans.unit.h": { en: "h", ja: "時間" },
  "loans.unit.d": { en: "d", ja: "日" },
  "loans.ends": { en: "ends {rel}", ja: "終了 {rel}" },
  "loans.endsPast": { en: "clock ran out {rel} — expires on next read", ja: "期限切れ {rel} — 次回読み込み時に満了" },
  ...
  "loans.timeline.title": { en: "Timeline", ja: "履歴" },
  "loans.timeline.empty": { en: "No events recorded for this loan.", ja: "この貸し出しの履歴はありません。" },
  "loans.timeline.notMigrated": {
    en: "The timeline table does not exist on this project yet. Apply {file} in the Supabase SQL editor, then reload.",
    ja: "このプロジェクトには履歴テーブルがまだ存在しません。Supabase の SQL エディタで {file} を適用してから再読み込みしてください。",
  },
  "loans.timeline.rpNote": {
    en: "RP split totals live on the row (rp_to_lender / rp_to_borrower). The ledger's loan_lender_share rows carry the borrower's name, not the loan id, so they are not listed here.",
    ja: "RP 分配の累計は行に保持されています（rp_to_lender / rp_to_borrower）。台帳の loan_lender_share 行には貸し出し ID ではなく借り手の名前が入っているため、ここには表示しません。",
  },
  "loans.timeline.created": { en: "created", ja: "作成" },
  "loans.actor.lender": { en: "lender", ja: "貸し手" },
  "loans.actor.borrower": { en: "borrower", ja: "借り手" },
  "loans.actor.system": { en: "system", ja: "システム" },
  "loans.actor.admin": { en: "admin", ja: "管理者" },
  "loans.ids": { en: "ids", ja: "ID" },

  "loans.action.force_return": { en: "Force return", ja: "強制返却" },
  "loans.action.cancel_offer": { en: "Cancel offer", ja: "オファー取消" },
  "loans.action.clear_cooldown": { en: "Clear cooldown", ja: "クールダウン解除" },
  "loans.dialog.force_return.title": { en: "Force-return this loan?", ja: "この貸し出しを強制返却しますか？" },
  "loans.dialog.force_return.body": {
    en: "The loan becomes RETURNED now, exactly as if the borrower had returned it: level_at_end is read from the lender's progress row. The borrower's client removes the asset on its next refresh. The lender keeps every RP already paid.",
    ja: "借り手が返却した場合とまったく同じ扱いで、貸し出しは即座に「返却済」になります（level_at_end は貸し手の進行データから取得）。借り手側のクライアントは次回の更新でアセットを削除します。すでに支払われた RP はそのまま貸し手のものです。",
  },
  "loans.dialog.cancel_offer.title": { en: "Cancel this offer?", ja: "このオファーを取り消しますか？" },
  "loans.dialog.cancel_offer.body": {
    en: "The offer becomes RESCINDED and the asset unlocks on the lender's side. Like a real rescind, this starts the pair's 24 h re-offer cooldown — clear it afterwards if the case calls for it.",
    ja: "オファーは「取り消し」になり、貸し手側のアセットのロックが解除されます。通常の取り消しと同様、この 2 人の間に 24 時間の再オファー・クールダウンが発生します。必要であれば続けて解除してください。",
  },
  "loans.dialog.clear_cooldown.title": { en: "Clear the cooldown for this pair?", ja: "この 2 人のクールダウンを解除しますか？" },
  "loans.dialog.clear_cooldown.body": {
    en: "The lender may offer this borrower again immediately. The declined / rescinded history stays; only its timestamp is moved out of the 24 h window.",
    ja: "貸し手はこの借り手に対して即座に再オファーできるようになります。辞退・取り消しの履歴は残り、そのタイムスタンプだけが 24 時間の範囲外に移動されます。",
  },
  "loans.dialog.note": { en: "Note (required)", ja: "メモ（必須）" },
  "loans.dialog.notePlaceholder": { en: "why — ticket id, what the player reported", ja: "理由 — チケット番号、プレイヤーからの報告内容など" },
  "loans.dialog.noteHint": {
    en: "At least 3 characters. Goes on the loan's timeline and into the audit log.",
    ja: "3 文字以上。貸し出しの履歴と監査ログに記録されます。",
  },
  "loans.dialog.confirm": { en: "Confirm", ja: "実行する" },

  "loans.rules.title": { en: "Rules", ja: "ルール" },
  "loans.rules.body": {
    en: "Read from the API's own constants (routers/loans.py) on every load, so this card cannot drift from the code. Tuning them here is deliberately not offered.",
    ja: "読み込みのたびに API 自身の定数（routers/loans.py）から取得しているため、このカードがコードとずれることはありません。ここからの変更は意図的に提供していません。",
  },
  "loans.rules.share": { en: "Lender's share", ja: "貸し手の取り分" },
  "loans.rules.days": { en: "Durations", ja: "貸出期間" },
  "loans.rules.out": { en: "Max loans out", ja: "貸出上限" },
  "loans.rules.in": { en: "Max loans in", ja: "借入上限" },
  "loans.rules.pending": { en: "Max pending offers", ja: "保留オファー上限" },
  "loans.rules.ttl": { en: "Offer TTL", ja: "オファー有効期限" },
  "loans.rules.cooldown": { en: "Re-offer cooldown", ja: "再オファー・クールダウン" },
  "loans.rules.endedWindow": { en: "Ended loans shown to clients", ja: "クライアントへの終了済表示期間" },
  "loans.rules.hours": { en: "{n} h", ja: "{n} 時間" },
  "loans.rules.daysFmt": { en: "{n} d", ja: "{n} 日" },
  "loans.rules.source": { en: "from {url}", ja: "取得元: {url}" },
  "loans.rules.unavailable": {
    en: "The API could not be reached ({reason}). The rest of this panel works without it.",
    ja: "API に接続できませんでした（{reason}）。このパネルの他の機能は影響を受けません。",
  },

  // ---- Users drawer → Loans tab (loans_ops §3.4) --------------------------
  "udrawer.tab.loans": { en: "Loans", ja: "貸し出し" },
  "users.loans.switch": { en: "Loan offers", ja: "貸し出しオファーの受付" },
  "users.loans.switchHint": {
    en: "profiles.golfin_loan_offers — OFF refuses every offer to this player with not_accepting, instantly. The player's own Settings switch writes the same column.",
    ja: "profiles.golfin_loan_offers — OFF にすると、このプレイヤーへのオファーはすべて即座に not_accepting で拒否されます。プレイヤー自身の設定画面のスイッチも同じ列を書き換えます。",
  },
  "users.loans.switchOn": { en: "ON", ja: "受付中" },
  "users.loans.switchOff": { en: "OFF", ja: "停止中" },
  "users.loans.turnOn": { en: "Turn on", ja: "オンにする" },
  "users.loans.turnOff": { en: "Turn off", ja: "オフにする" },
  "users.loans.switch.on.title": { en: "Turn loan offers ON for this player?", ja: "このプレイヤーの貸し出しオファー受付をオンにしますか？" },
  "users.loans.switch.on.body": {
    en: "Other players can offer them loans again, immediately.",
    ja: "他のプレイヤーが即座にこのプレイヤーへオファーできるようになります。",
  },
  "users.loans.switch.off.title": { en: "Turn loan offers OFF for this player?", ja: "このプレイヤーの貸し出しオファー受付をオフにしますか？" },
  "users.loans.switch.off.body": {
    en: "Every new offer to them is refused with not_accepting, immediately. Pending offers and running loans are not touched.",
    ja: "このプレイヤーへの新規オファーはすべて即座に not_accepting で拒否されます。保留中のオファーと進行中の貸し出しには影響しません。",
  },
  ...
  // ---- Telemetry → Loans section (loans_ops §2) ---------------------------
  "tel.tab.loans": { en: "Loans", ja: "貸し出し" },
  "tel.loans.title": { en: "Loans — is lending used, and how", ja: "貸し出し — 利用状況と流れ" },
  "tel.loans.funnelTitle": { en: "Client funnel (telemetry events)", ja: "クライアントのファネル（テレメトリイベント）" },
  "tel.loans.funnelHint": {
    en: "loan_modal_open → loan_offer_sent → loan_offer_answered → accept → loan_return, as the client reported them. Each bar is a share of modal opens. The lifecycle numbers below come from the server's own rows and are the authority on what happened; this strip shows who looked and did not send.",
    ja: "loan_modal_open → loan_offer_sent → loan_offer_answered → 承諾 → loan_return を、クライアントの報告どおりに並べたものです。各バーはモーダル表示数に対する割合です。下のライフサイクル数値はサーバー自身の行から算出した正となる値で、このファネルは「見たが送らなかった」人を示します。",
  },
  "tel.loans.stage.modalOpens": { en: "Lend modal opened", ja: "貸し出しモーダル表示" },
  "tel.loans.stage.offersSent": { en: "Offer sent", ja: "オファー送信" },
  "tel.loans.stage.answered": { en: "Offer answered", ja: "オファー回答" },
  "tel.loans.stage.accepted": { en: "Accepted", ja: "承諾" },
  "tel.loans.stage.returned": { en: "Returned early", ja: "早期返却" },
  "tel.loans.funnelSub": {
  ...
  "tel.loans.viaSearchHint": {
    ...
    ja: "loan_offer_sent.via = search ÷ 送信数 — フォローしていない相手へのオファーの割合です。",
  },
  "tel.loans.lifecycleTitle": { en: "Lifecycle (golfin_loans in range)", ja: "ライフサイクル（期間内の golfin_loans）" },
  "tel.loans.kpi.pending": { en: "Pending now", ja: "現在保留中" },
  "tel.loans.kpi.pendingHint": { en: "offered, and the 48 h offer clock has not run out at this moment — whatever the status column says.", ja: "オファー中で、この時点で 48 時間の期限が切れていないもの（ステータス列の値に関わらず）。" },
  "tel.loans.kpi.active": { en: "Active now", ja: "現在貸出中" },
  "tel.loans.kpi.activeHint": { en: "active and now < ends_at — the router's own live predicate, applied to the timestamps.", ja: "active かつ now < ends_at — ルーターと同じ判定をタイムスタンプに適用しています。" },
  "tel.loans.kpi.acceptRate": { en: "Accepted", ja: "承諾率" },
  "tel.loans.kpi.acceptRateSub": { en: "{accepted} of {answered} answered", ja: "回答 {answered} 件中 {accepted} 件" },
  "tel.loans.kpi.acceptRateHint": { en: "accepted ÷ (accepted + declined). Offers still pending, rescinded or lapsed are not answers.", ja: "承諾 ÷（承諾 + 辞退）。保留中・取り消し・失効したオファーは回答に含めません。" },
  "tel.loans.kpi.earlyReturn": { en: "Returned early", ja: "早期返却率" },
  "tel.loans.kpi.earlyReturnSub": { en: "{n} of {ended} ended", ja: "終了 {ended} 件中 {n} 件" },
  "tel.loans.kpi.earlyReturnHint": { en: "returned with ended_at before ends_at ÷ (returned + expired).", ja: "ends_at より前に返却された件数 ÷（返却済 + 期限満了）。" },
  "tel.loans.kpi.rpLenders": { en: "RP to lenders", ja: "貸し手への RP" },
  ...
  "tel.loans.col.share": { en: "Share", ja: "割合" },
  "tel.loans.daysTitle": { en: "Duration mix", ja: "期間の内訳" },
  "tel.loans.kindTitle": { en: "Kind mix", ja: "種類の内訳" },
  "tel.loans.topLenders": { en: "Top lenders", ja: "貸し手トップ" },
  "tel.loans.topBorrowers": { en: "Top borrowers", ja: "借り手トップ" },
