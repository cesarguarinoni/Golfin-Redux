...
- **ALWAYS-ON text gate — font WEIGHT + rendered SIZE-vs-reference (standing rule, Cesar 2026-07-01; every Figma-node task, unconditional).** `1v1_result_rewards_display` cleared this gate AND the red-team, then Cesar rejected it for text problems no gate checked: bold labels that should be regular, a non-bold "Vs." that should be bold, non-bold usernames, and fonts that matched a `node_px ÷ divisor` number but rendered visibly too small vs the reference. So every text row in your `## Figma fidelity` table must carry, and you must independently verify, TWO extra columns: (1) **Weight** — built (Bold/SemiBold/Medium/Regular) vs node weight; a mismatch is a FAIL row on its own. (2) **Rendered size vs reference** — never PASS a size just because `node_px ÷ 1.2` (or any divisor) matches a number; A/B the RENDERED cap-height against the `reference/` node render at matched scale and FAIL if it looks smaller/larger than the reference, regardless of the arithmetic. The reference render is ground truth for visual size. Run this on EVERY Figma-node task, even when no text problem was flagged.
...
### Step 2d — UI fidelity lint (re-run yourself, Rule 21)

For a Figma-node UI task, do NOT trust the implementer's `## UI fidelity lint` numbers — **re-run the linter yourself this pass** via `script-execute`: `Golfin.EditorTools.UIFidelity.UIFidelityLinter.LintPrefab("Assets/…/X.prefab", "<spec.json>")` for every new/changed prefab, and paste the report into `ARCHITECT_REVIEW.md`. It is a deterministic render-health + node-spec pass: `fail == 0` is required to advance. Any `FAIL` (9-slice collapse→oval, non-9-slice corner distortion, flat-fill where a sprite is required, size/radius/gap/color/font off-spec) = `ARCHITECT_REVIEW_FAIL`, no qualitative override — the linter caught the `PillFill` corner-collapse on `stamina_boost_shop` that BOTH the implementer and Cesar missed by eye. This is the automated backstop under the visual gates: Step 2b says it LOOKS right, this says the geometry/sprites objectively ARE right.
