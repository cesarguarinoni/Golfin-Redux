...
- **§9b text weight + rendered-size gate (ALWAYS-ON, standing rule Cesar 2026-07-01):** for EVERY text element, verify built font WEIGHT (Bold/SemiBold/Medium/Regular) against the node weight — a mismatch is `ARCHITECT_REVIEW_FAIL`; and A/B the RENDERED cap-height against the `reference/` render at matched scale — FAIL if the built text looks smaller/larger than the reference, even when `node_px ÷ divisor` math "matches" (the reference render is ground truth for visual size, not the arithmetic). `1v1_result_rewards_display` passed this gate then Cesar rejected it for exactly these (bold-vs-regular labels/Vs./usernames, and too-small fonts that matched a divisor). Run it every Figma-node task, unconditionally.
...
- **§12 UI fidelity lint re-run (Rule 21):** re-run `Golfin.EditorTools.UIFidelity.UIFidelityLinter.LintPrefab(...)` on every new/changed prefab yourself via `script-execute` — do NOT trust the report's or reviewer's cited `_lint.json`. Any `fail > 0` (9-slice collapse→oval, non-9-slice corner distortion, flat-fill fabrication, size/radius/gap/color/font off-spec) = `ARCHITECT_REVIEW_FAIL`. This is the deterministic gate for the oval-pill / BUY-radius / dark-tint / wrong-gap class Cesar caught by eye on `stamina_boost_shop`; the render-health layer needs no reference, so there is no excuse to skip it.
(`PIPELINE_HARDENING.md` §9–§12. Scar: `tournament_signup_modal` — a flat-colour spriteless modal with fabricated clone PASSes cleared the self-review + reviewer gates; only Cesar's eyeballs caught it, repeatedly.)
