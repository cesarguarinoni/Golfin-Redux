...
namespace Golfin.CourseImport
{
    public static class HoleLiteImporter
    {
        ...
        /// Uses centroid-fan triangulation with edge-midpoint stripe assignment
        /// so geometry never extends outside the polygon boundary.
        /// </summary>
            ...
            Vector2 stripeDir, float stripeWidth)
        {
            ...
            // Use perimeter edge midpoint (not triangle centroid) for band assignment
            // so stripes follow the polygon boundary correctly
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                // Midpoint of the perimeter edge (ignoring centroid vertex)
                float edgeMidX = (worldPts[i0].x + worldPts[i2].x) * 0.5f;
                float edgeMidZ = (worldPts[i0].z + worldPts[i2].z) * 0.5f;

                float proj = edgeMidX * stripeDir.x + edgeMidZ * stripeDir.y;
                int band = Mathf.FloorToInt(proj / stripeWidth);

                var target = (band % 2 == 0) ? lightTris : darkTris;
                target.Add(i0);
                target.Add(n); // centroid
                target.Add(i2);
            }
