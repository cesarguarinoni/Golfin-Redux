// ─────────────────────────────────────────────────────────────────────────────
// NoticesRuntime — RemoteNoticeSource
// Fetches the live notice set over ApiClient and mirrors the RAW response body
// to disk. Shape-for-shape the same as RemoteBannerSource, for the same
...
{
    /// <summary>Where the notice set the game is currently showing came from.</summary>
    public enum NoticeSource
    {
        /// <summary>Nothing fetched and nothing cached — the Home notice panel is hidden.</summary>
        None,
    ...
    public static class RemoteNoticeSource
    {
        private const string Tag = "[Notices]";
        public const string CacheFileName = "home_notices.json";

        /// <summary>
        /// <c>&lt;persistentDataPath&gt;/home_notices.json</c>.
        ...
        public static string? ReadCache()
        {
            ...
            {
                Debug.LogWarning($"{Tag} Could not read the notice cache: {ex.Message}");
                return null;
            }
        }
        ...
        public static void WriteCache(string json)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogWarning($"{Tag} Could not write the notice cache '{path}': {ex.Message}");
            }
        }

        /// <summary>Test/debug helper — drop the cache so the next boot starts with no notices.</summary>
        public static void ClearCache()
        {
            try { if (File.Exists(CachePath)) File.Delete(CachePath); }
            catch (Exception ex) { Debug.LogWarning($"{Tag} Could not delete the notice cache: {ex.Message}"); }
        }

        // ── Fetch ─────────────────────────────────────────────────────────────

        /// <summary>
        /// GET the live notice set. On success the RAW body is mirrored to disk BEFORE mapping, so
        /// even a payload this build cannot map is available to a later build that can.
        /// <paramref name="onDone"/> receives null on any failure — the caller keeps its current
        /// set, which on a cold offline launch means the panel simply stays hidden.
        /// </summary>
        public static IEnumerator FetchRoutine(Action<string?> onDone)
        {
            string url = Endpoints.Notices;
            string? body = null;
            ...
            IEnumerator get = ApiClient.Instance.Get<string>(url, result =>
            {
                ...
                {
                    // Expected on a cold launch in airplane mode. Warning, not error: hiding the
                    // panel is a designed path, not a malfunction.
                    Debug.LogWarning(
                        $"{Tag} Notice fetch failed ({result.ErrorKind}, HTTP {result.StatusCode}): " +
                        $"{result.ErrorMessage}. Keeping the cached notices.");
                }
            });
