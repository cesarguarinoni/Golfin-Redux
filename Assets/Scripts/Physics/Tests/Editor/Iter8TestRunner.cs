...
/// <summary>
/// §controls_h iter-8 fallback test runner. Runs the Physics.Tests EditMode suite
/// and writes results to /tmp/iter8_test_results.txt.
/// Invoked via: GOLFIN > Run Physics Tests (Iter8)
/// </summary>
public static class Iter8TestRunner
{
    [MenuItem("GOLFIN/Run Physics Tests (Iter8)")]
    public static void Run()
    {
        ...
        Debug.Log("[Iter8TestRunner] Test run started.");
    }
    ...
    sealed class Callbacks : ICallbacks
    {
        ...
        public void RunStarted(ITestAdaptor testsToRun)
            => Debug.Log($"[Iter8TestRunner] Starting. Total scheduled: {CountLeaf(testsToRun)}");

        public void RunFinished(ITestResultAdaptor result)
        {
            string summary = $"PASS:{_pass} FAIL:{_fail} SKIP:{_skip}\n";
            string fullText = summary + _sb.ToString();
            File.WriteAllText("/tmp/iter8_test_results.txt", fullText);
            Debug.Log($"[Iter8TestRunner] COMPLETE — PASS:{_pass} FAIL:{_fail} SKIP:{_skip}");
            if (_fail > 0)
                Debug.LogWarning($"[Iter8TestRunner] FAILURES:\n{_fails}");
        }
