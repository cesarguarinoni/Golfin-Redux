...
/// <summary>
/// §2c test runner. Runs the Physics.Tests EditMode suite
/// and writes results to /tmp/iter2c_test_results.txt.
/// Invoked via: GOLFIN > Run Physics Tests (Iter2c)
/// </summary>
public static class Iter2cTestRunner
{
    [MenuItem("GOLFIN/Run Physics Tests (Iter2c)")]
    public static void Run()
    {
        ...
        Debug.Log("[Iter2cTestRunner] Test run started.");
    }
    ...
    sealed class Callbacks : ICallbacks
    {
        ...
        public void RunStarted(ITestAdaptor testsToRun)
            => Debug.Log($"[Iter2cTestRunner] Starting. Total scheduled: {CountLeaf(testsToRun)}");

        public void RunFinished(ITestResultAdaptor result)
        {
            string summary  = $"PASS:{_pass} FAIL:{_fail} SKIP:{_skip}\n";
            string fullText = summary + _sb.ToString();
            File.WriteAllText("/tmp/iter2c_test_results.txt", fullText);
            Debug.Log($"[Iter2cTestRunner] COMPLETE — PASS:{_pass} FAIL:{_fail} SKIP:{_skip}");
            if (_fail > 0)
                Debug.LogWarning($"[Iter2cTestRunner] FAILURES:\n{_fails}");
        }
