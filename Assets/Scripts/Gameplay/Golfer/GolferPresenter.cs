...
namespace Golfin.Gameplay.Golfer
{
    ...
    public sealed class GolferPresenter : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            //
            // A no-op unless forceGripPose is set (PfGolfer_Test's wrap). It does not read the
            // club's transform back, so posing the hands here cannot move the club — Animation
            // Rigging has already evaluated GripTarget inside the animator graph. The Mixamo
            // grip is SPEC §3.12's HandHingeModel (its own component), not a branch here.
            ApplyGripPose();
