...
//
// ⚠️ GOLFIN: THIS .shader TEXT IS THE SOURCE OF TRUTH — DO NOT REGENERATE FROM THE ASE GRAPH.
//    Hand edits live between `// ── GOLFIN OCCLUDE FADE ──` … `// ── END GOLFIN OCCLUDE FADE ──`
//    markers (1 helper block in HLSLINCLUDE + 1 identical injection in each of Forward, DepthOnly,
//    DepthNormals, GBuffer, Universal2D). An Amplify regen WILL silently wipe all six.
//    Spec: Docs/Specs/Active/tree_occlusion_fade/SPEC.md §4.2
Shader "Custom/Vegetation"
{
	...
	SubShader
	{
		...
		// ── GOLFIN OCCLUDE FADE ── (tree_occlusion_fade spec §4.2)
		// !! This file is Amplify-generated. From now on the .shader TEXT is the source of truth:
		// !! regenerating it from the ASE graph WILL wipe every block between these markers.
		// !! Injected in 5 passes: Forward, DepthOnly, DepthNormals, GBuffer, Universal2D.
		// !! Deliberately NOT in ShadowCaster (a faded tree keeps its ground shadow) or Meta (bake-only).
		// Globals are published per-frame by Golfin.Physics.Viewer.TreeOccludeFadeDriver.
		float4 _GolfinOccFadeBall;    // xyz = focus (ball) world pos
		float4 _GolfinOccFadeCam;     // xyz = gameplay camera world pos
		...
		// NOTE: this block sits in HLSLINCLUDE, i.e. BEFORE each pass pulls in URP's UnityInput.hlsl,
		// so `_WorldSpaceCameraPos` is not declared here yet. The driver therefore publishes the camera
		// position as its own global — keeping this block dependency-free and identical for every pass.
		float GolfinOccFadeAmount(float3 worldPos)
		{
			float s = _GolfinOccFadeStrength;
			if (s <= 0.001) return 0.0;
			float3 toBall = _GolfinOccFadeBall.xyz - _GolfinOccFadeCam.xyz;
			float3 toFrag = worldPos               - _GolfinOccFadeCam.xyz;
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				// ── GOLFIN OCCLUDE FADE ── (tree_occlusion_fade §4.2 — identical in all 5 patched passes)
				float golfinFade = GolfinOccFadeAmount(WorldPosition);
				if (golfinFade > 0.001)
					clip((1.0 - golfinFade * _GolfinOccFadeParams.z) - GolfinBayer4(uint2(IN.clipPos.xy)));
				// ── END GOLFIN OCCLUDE FADE ──
