...
- A2 / A4 the board's real CloseButton → `TournamentSelection` — **recorded as finding F1 instead**:
  an empty board hides the button, so the bot leaves through the real bottom-nav TEE
...
## Run log — 2026-09-14 14:44 JST, Editor 6000.3.9f1, signed-in dev profile, schedule from the disk cache

`route_verify.json` → **PASS**, 11/11 route checks (the bot also CREATES the reported state:
...
- **F1 — an empty tournament board has no CLOSE.** `TournamentCloseButton` lives at
  `Bottom97/ScrollArea/Viewport/GridContent/CloseSlot`, and `ApplyBoardChrome` deactivates the whole
  `ScrollArea` when `rankedCount == 0`. Every ENDED tournament with no finishers — i.e. exactly the
  boards the fixed LEADERBOARD button now opens — can only be left through the nav bar. Still:
  `A1_leaderboard_from_ended_card.png` ("No finishers yet", no CLOSE).
- **F2 — a schedule applied after sign-in never reaches the wrapper the session plays on.**
...
- **F3 — the hole-selection header is authored text.** "SPONSORED BY PUMA / KASUMIGASEKI OPEN" and the
  "Lomond Country Club" label prefix are not bound to the selected tournament (only the ENDS-IN pill
  and "Hole N - Par P" are). Visible in `B1_…png` for `kisarazu_cup`.
