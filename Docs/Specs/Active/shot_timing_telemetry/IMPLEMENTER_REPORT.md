...
| A6 | §23: `npm run deploy`, deployment id quoted, live commit stamp matches HEAD | **PASS (id) / MANUAL (stamp)** | `Current Version ID: cc9b9dd3-4e00-4598-b0cf-4d6a692f0999`, `Uploaded golfin-admin (8.40 sec)`, `admin.golfin.world (custom domain)`. Tests gate ran first and passed. The tree was committed **before** deploying, so the build stamped `c77c7732` and not `…-DIRTY` (the script prints the DIRTY warning only when `git status --porcelain -- .` is non-empty; it was empty). Access still protects the site: `curl -o /dev/null -w %{http_code}` → **302**. Reading the stamp off the live sidebar needs the same Access login as A5. |
