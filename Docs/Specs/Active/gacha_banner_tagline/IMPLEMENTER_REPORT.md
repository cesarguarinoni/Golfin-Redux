...
**Incident during the chain — my misread, corrected here:** ~10 s after my `wk_2026_39` MATERIALIZE, drafts for `wk_2026_40` (04:20:39 UTC) and `wk_2026_41` (04:21:10) appeared under the same signed-in account and the Worker returned Cloudflare 1102 for ~30 s. I read that as the "Materialize again?" dialog re-firing on the auto-advanced selection and, before publishing the chain, deleted the 64 draft-only rows it had created (13+13 shop, 6+6 rates, 12+12 pools, 2 banners; none published) through the PostgREST path `import_content.py` writes drafts with, and reset the two rotation stamps. **That diagnosis was wrong.** The audit log shows it was a PARALLEL SESSION (`weekly_banners_to_admin`, same Chrome profile → same admin account) materializing those weeks on purpose and uploading their bespoke art; it re-materialized wk_40 + wk_41 at 04:26:30–04:27:04 and uploaded their art at 04:27:34 / 04:28:09, so its drafts are intact now (13/6/12/1 rows each, `artUrl` set, stamps 04:26:30Z / 04:26:56Z — verified via REST after the fact). Net effect of my deletion: that session had to redo two materializes; nothing was published or lost. Its wk_39 art upload (04:24:36, `…-91666a8bdf37.jpg`) landed on the wk_39 banner draft one minute before my chain publish, so `gacha_banners` v16 carries that art — which is what its own record says it wanted for weeks 38–39. Disclosed to Cesar in the wrap-up; no dashboard defect to file (the earlier "re-fire" hypothesis is withdrawn). Lesson: a shared admin account cannot be attributed by `admin_email` — check for a parallel session's record (`Docs/AI_CONTEXT.md`, the peers list) before treating unexpected drafts as a runaway.
...
- ~~Dashboard defect ("Materialize again?" re-fires)~~ — withdrawn: the extra materializes were a parallel session's deliberate work (see § Implementation summary), not a re-fire.
