...
**Two spec points that could not ship as written** (details under § Deviations): the ball
listing is **one ball**, not ten (`golfin_shop_purchase` reads `quantity` for tickets only and
rule G3-Q refuses anything else on a ball row); and the pity E2E on prod (§6.4) **waits on the
migration**, which is DDL and therefore Cesar's SQL-editor step.
...
| Pity migration: verification block passes; live E2E in §6.4 quoted | **FAIL** (blocked on the DDL apply — Cesar) | The migration is written, mirrored, committed (`df84963`) and parse-checked (pglast: SQL + function body + DO block); it is DDL (`create or replace function`) and cannot be applied over PostgREST — full SQL is in the chat for the SQL editor, Part 1 (function) then Part 2 (verification, self-rolling-back, expected `after A x3: counter=3 key=weekly` / `after B x1: counter=4 key=weekly` / `PASS`). Until it is applied prod keys pity by banner id, so pulling on two grouped banners would NOT continue — the live §6.4 run is pointless before the apply and is queued for the moment Cesar confirms (I create two short test rotations, pull on the first twice, on the second once, quote `golfin_gacha_pity`, then deactivate both). |
...
**Database:** rotations seed **applied** (data, PostgREST). Pity migration **waiting on Cesar**
(DDL).
...
- **Apply `2026_09_11_gacha_pity_group.sql`** (Part 1, then Part 2 as its own query) — then the
  live §6.4 pity E2E, which I run and quote.
