...
| Hole Indicator | `14123:32491` (frame "Hole Indicator (tail up)") | The in-game `HoleIndicator` chip + arrow line (prefab visuals used by `HoleIndicatorWidget`), distance text = ball → pin in the active unit. Replaces the code-built 48 px yellow `FlagIcon`. **The tail always points AT the hole**: the line is solid at the chip and fades to transparent toward the pin (the prefab's existing gradient — just oriented so the FADING end is the one on the pin's screen point; no dot, no arrowhead), and the 100×100 chip hangs at the OTHER end of the line, on the side of the pin that has room — below the pin when the pin is in the top half of the screen (the normal map case, Figma), above it otherwise. **Minimum tail length `_pinTailMinPx = 120`** (serialized), so the chip never covers the hole; the tail grows beyond that only if the chip would otherwise leave the safe area. When the pin is off-screen it docks to the edge exactly as today (Order 355 `_flagArrowRT` logic) — that path keeps its current sprites. |
...
Replace the body of `BuildHoleIndicator`'s `FlagIcon` block: instantiate the `HoleIndicator` prefab visuals (chip + `_arrowLine`) under `_indicatorCanvas`, driven by a small `MapPinIndicator` helper. Each frame: `pinScreen = WorldToScreenPoint(pin)`; the line's FADING end at `pinScreen` (rotate the `_arrowLine` so its transparent end is the pin end — the prefab gradient runs solid-at-chip → clear-at-tip already); `dir = pinScreen.y > Screen.height/2 ? down : up` (chip goes to the side with room); `tailPx = max(_pinTailMinPx, whatever keeps the chip inside the safe area)`; line from `pinScreen` to `pinScreen + dir·tailPx`, chip anchored at that end (chip edge touching the line, centred on it). `_distanceText = FormatDistance(|ball − pin|)`. The chip must NEVER overlap the pin point — add `Assert(Vector2.Distance(chipRect.ClosestPoint(pinScreen), pinScreen) >= _pinTailMinPx − chipHalf)` to `DumpInvariants`. Keep `_flagArrowRT` + the Order 355 edge-docking; the docked state may keep the current arrow sprite. NOTE: if the HUD's `HoleIndicator` is a scene object rather than a prefab, extract it to a prefab first (one scene edit, listed in the report) — do not copy its hierarchy by hand.
...
- [ ] Hole Indicator tail points at the pin in every capture; the chip never covers the pin (invariant quoted); flips above the pin when the pin is in the lower half (force it by pinching/panning).
- [ ] All `[SerializeField]` references wired in the Inspector (`_shotViewButton`, `_guideDotWorldWidth`, `_rangeFanHalfAngleDeg`, `_pinTailMinPx`)
