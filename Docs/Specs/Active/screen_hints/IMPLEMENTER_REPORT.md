...
| Stacking: with a tournament result pending on Home (`TournamentResultPresenter` fixture), the result modal opens first and the hint opens after it closes — order quoted | FAIL | No tournament-result fixture exists in the repo, so the stack was exercised with `SchemeConfirmModal` open before `Leaderboard`: `STACK: 1.5s after Leaderboard: hint visible=False openModals=1` → `SchemeConfirmModal hidden` → `modal visible for Leaderboard after stack after 0.02s` (`leaderboard_hint_waiting_behind_modal.jpg`, `leaderboard_hint_after_stack.jpg`) — the wait-behind-a-modal mechanism works. But the ORDER the item asserts would not hold with `TournamentResultPresenter`: it presents 1.0 s after `ScreenChanged` (`_settleDelay`), the hint presents one frame after (Architect default c), so on Home the hint opens FIRST and the result modal waits on `ModalStackEmptied` until the hint closes. Architect decision needed — § Open questions |
...
- **Stacking order vs `TournamentResultPresenter`.** The mechanism (wait behind any open `ModalController`,
  resume on `ModalStackEmptied`) is proven with a stacked `SchemeConfirmModal`. The specific order the item
  asserts — result modal first, hint after — is not what the two presenters produce together: the tournament
  presenter waits `_settleDelay = 1.0 s` before opening, the hint presenter waits one frame (Architect default c),
  so the hint wins and the result follows it. Unblock = an Architect call: (a) accept hint-first (no code change —
  the result still opens, right after CLOSE), or (b) have `ScreenHintPresenter` wait ≥ 1.1 s on the four
  tournament-eligible screens so the result goes first (one constant), or (c) a general "wait 1.1 s everywhere"
  which delays every hint. No fixture to run the real result modal was found (`TournamentResultModalController`
  needs a backend result); if one exists, point me at it and I will re-run the bot against it.
...
- **Hint vs tournament result order** (the FAIL above): accept hint-first, or make the hint presenter wait past
  `TournamentResultPresenter._settleDelay` on its four screens? Today's code is default (c) as written.
