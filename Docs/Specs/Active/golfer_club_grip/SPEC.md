...
| A3 | Harness run — ONE, Mixamo-native, Hole 06 (deterministic step, §3.9.5) | **§3.9.6 rows all PASS at all three samples**; `grip.hand.onShaft_l/_r` (palm point) < 0.035 m worst-of-three; `grip.hand.orient_l/_r` < 5°; `grip.hands.order` in band; `club.headAtBall` < 0.05 m; `grip.ikNoLegEffect` in band; every §9.8 PASS still PASS; `budget.tris` still the one FAIL (unchanged, out of scope) |
