...
5. **Run all 18 holes.** Per-hole rows plus an aggregate. A per-hole outlier is a finding, not noise — Hole 15 in particular inverted the failure last time (its fairway classified as `Green`), so report it individually.
   ...
   **Hole 02 — SUPERSEDED NOTE, READ CAREFULLY.** An earlier revision of this spec told you Hole 02 has zero OB and would show the largest fallthrough share of any hole. **That is no longer true and you must not calibrate against it.** Hole 02's `zones.png` had never had the OB colour painted (the only hole of 18); it was repainted, re-exported, re-imported and re-baked on 2026-07-29 (`4b0054069`). Its runtime `obMask` now carries **760,542 / 1,048,576 = 72.5% set bits**, up from 0, and its `tree_obstacles.csv` lost 331 rows. **Treat Hole 02 as an ordinary hole.** If your numbers still show it with zero OB coverage or an outlier fallthrough share, you are reading stale baked data — `git pull` and re-check `Assets/Resources/HoleData/lomond-country-club/Hole_02/zones.json` before reporting anything. Background: `Docs/SURFACE_CLASSIFICATION_PIPELINE.md` §6.1.
