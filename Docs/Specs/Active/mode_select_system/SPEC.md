...
- **Locked card** -> reuse the **locked-hole treatment** (darker card + lock glyph in title + "Coming soon", PLAY disabled). NOTE-L: a quick grep found no `Locked` symbol in `HoleCardController`, so the locked state may be Figma-only / data-driven. Code verifies and lifts the real treatment, or builds it from the Figma locked-hole frame if not yet in code.
