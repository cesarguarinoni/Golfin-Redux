...
- ✅ **RESOLVED (Cesar, 2026-08-17): bots pad the board early, then disappear once enough humans have entered.** Not a blended board with a euphemistic prize label, and not a permanently-split list — the field is filler, and filler should stop being shown once it is no longer needed.
  - **Prizes are unaffected either way**: they always follow the human-only ordering, in every state.
  - The spec that implements this must settle three things the decision does not by itself answer: (1) the threshold N and whether it is absolute or a fraction of `bot_count`; (2) whether hidden bots are merely hidden from display or also removed from the ranking — **removed is strongly preferred**, because hiding-but-still-ranking leaves visible rank gaps and keeps rank ≠ prize, which is the confusion this decision exists to kill; done that way the board converges on "rank == prize rank" exactly when it has enough real players to look healthy; (3) the transition must not yank a player's displayed rank mid-round — decide whether the switch is evaluated live or frozen at entry/resolve.
