...
- **Stage 2** — `SaveBackedEntryStore` adapter (map ⇄, upsert, `IsClaimed`/`MarkClaimed`, `MarkDirty`) + store round-trip / upsert / **claim-persists-across-restart** / debounce / restart tests.
...
- **D2 — ✅ LOCKED (b): persist claim state.** Claim-once survives app restart — no relaunch double-claim. T4 grows `ITournamentEntryStore` with `IsClaimed`/`MarkClaimed` (in-memory fake = `HashSet`); T5's `SaveBackedEntryStore` implements them via the `claimed` column + `MarkDirty()`. Mirrored into the T4 spec (§2 seam, §4 `ClaimPrize`, §8 D-claim).
