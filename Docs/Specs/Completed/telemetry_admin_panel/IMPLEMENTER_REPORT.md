...
| 6. Live smoke: §3.1 totals match hand-run Supabase counts | **BLOCKED — needs real rows (Cesar)** | `telemetry_events` does not exist yet, so there is nothing to count. Re-run once the `beta_telemetry` §2.2 migration is applied and testers have played. |
