...
- [ ] Stroke 2 TOP_TOPSPIN: visibly lower apex than CENTER in flight (Magnus sign-flip; verified from captioned video). **Note (amended 2026-05-26 at architect review close-out):** original criterion required "≥8m further total" — that's incompatible with the current physics model, which implements Magnus aero lift but no spin→ground-roll velocity coupling (`BallSimulation.cs:264` zeros spin at first bounce; RunBouncePhase + RunRollPhase have no spin coupling). True topspin via sign-flip produces *downward* Magnus → shorter carry mechanically, not a tuning miss. v1 visual gate uses "lower apex" instead. A numeric `peakY` threshold can be added later if/when `ball_simulation_peak_y_logging` (P3 follow-up) lands.
...
- [ ] Stroke 5 RIGHT_FADE: ball curves right. Δ lateral ≥+5m vs CENTER **at first ground contact (or terminal if in-bounds)**. **Note (amended 2026-05-26):** original criterion read "vs CENTER terminal" — but FADE shots that go OOB get reset to tee, destroying the terminal measurement. First-bounce `[Land]` log line is the canonical curl evidence in that case. Iter-2 measured +12.5m body-frame right at land, well above the 5m bar.
