...
| Authored grip hand pose (fingers closed on the shaft) for the Mixamo-native golfer | `golfer_club_grip` §5 | One hand-pose clip layered on a Hands avatar mask, or a per-finger Animation Rigging chain; clips are empty-hand mocap and will never carry it |
