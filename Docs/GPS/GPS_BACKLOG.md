...
| Close-range golf grip — authored hand pose per roster model (artist deliverable, fingers-only layer; `reference/GOLF_GRIP_GEOMETRY.html` is the brief) | `golfer_club_grip` §3.11.5 (2026-09-11) | Only if a camera ever shows hands large (replay, showcase). Not procedural — nine rounds proved that. Stand-in Remy gets nothing further |
| Left-handed golfer mirror (swap `GripAnchor_Lead/Trail`, mirror the club local offsets, `rightHanded` flag) | `golfer_club_grip` §5 | After the right-handed grip is accepted on device |
| Clubface roll assertion in the golfer harness (`ClubStart→ClubEnd` roll vs the aim) | `golfer_club_grip` §5 | Only once the face is visible at gameplay-camera size |
| Remove the Quaternius finger-wrap solver (`GolferPresenter.ApplyGripPose` and friends) | `golfer_club_grip` §5 | Dead with the retarget branch; delete together with `PfGolfer_Test` when the first roster model lands |
| Remy stand-in is 36,510 tris (limit 15,000) | `golfer_3d_test` §9.8 | Not a shippable character; the budget check applies to the roster models, not to the pipeline proof |
