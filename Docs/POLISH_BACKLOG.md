...
| P-016 | **JA font binding** — 660 CJK labels render on a Rubik asset via TMP fallback; render is CORRECT today (red-team `REDTEAM_ja_MissionSelection.png`), binding is inconsistent (Stamina screens bind NotoSansJP deliberately). `LocalizedText` swaps the string, never the font (`japaneseFontScale` exists, no family hook) | `design_consistency_audit` Q1 — Cesar 2026-09-06: backlog, not a defect | swap `font` to `NotoSansJP-VariableFont_wght SDF` in `LocalizedText` on language change + a JA visual pass on every screen; pair with P-012 (Rubik Medium import) so fonts move once |
| P-017 | **Bars: `Image.Type.Filled` on 225 bar images** (`Bar` 182, `BarContainer` 33, `BarPending` 8, `GhostBar` 2 — StatBar family, durability, progress) — `Filled` discards 9-slicing, caps render as a wedge (playbook §3) | audit Q5 — deferred 2026-09-06 | width-driven bars (`sizeDelta.x`) via the shared `Bar()` pattern; `game_polish_b` tweens `fillAmount` until then |
| P-018 | **÷1.4 sizes on the ModeCard family** (209 labels: 27.86→39, 32.14→45 per node `13026:2366`; every label 28.6 % under, visible in `ModeSelectionScreen_sheet.png`) | audit Q7 — deferred 2026-09-06 | re-set to NODE values (never a divisor); one Quick when Cesar wants it — pairs naturally with Q3's card rebuild |
| P-019 | **÷1.2 sizes** (139 labels: 16.7 · 17 · 25 · 27.5 · 37.4 · 40 · 42.5 · 55 across Inventory, Roster, Settings, TournamentSignup, InGameSettings…) and **274 unexplained** sizes | audit Q7b + Q8 — deferred 2026-09-06 | per-site node comparison, batched per screen; needs `figma_node_to_spec.py` specs per screen (audit A5 was partial) |
| P-020 | **291 panel-sized flat fills** (710 visible null-sprite Images) — scrims are correct, missing art is not; triage per site against the node | audit Q6 — deferred 2026-09-06 | per-screen triage table (site → scrim / hairline / MISSING ART), then bakes per the palette |
| P-021 | **Audit gaps** — modals + Tier-2 dumped EN only, no modal crop sheets; shape (ix) card radius live side not measured; 9 of 17 screens re-seated via `ShowScreen` (no player path from a fresh session) | audit § 6 | a modal-only audit pass with JA + crop sheets when the modal pops (game_polish_b) land |
