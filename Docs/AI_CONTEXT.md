...
| Physics Architecture | ✅ Phases 0–6 COMPLETE; Phase 6 Stat Coupling COMPLETE; **BAKED-DATA SIM PIVOT COMPLETE (2026-04-25).** Sim now reads from baked `zones.json` (polygon classifier with bit-packed OB mask + per-zone triangulated mesh for exact barycentric Y interpolation) + `heightmap.bytes`. Original "ball into the void" repro eliminated by construction. Branch `sim-baked-data-path` ready for Phase E. M2 height agreement: **100/100 within 5 cm (mean 0.45 cm)**. M0 regression: 20/24 PASS, 4 directions Ignored pending the airborne ground-level-detection fix (queued spec: `Docs/Specs/Queued/AIRBORNE_GROUND_LEVEL_DETECTION.md`). Full M4 real-conditions suite: 228 / 212 PASS / 16 Skipped / 0 FAIL across 18 holes. Cesar's manual confirmation (Phase E) pending — see `Docs/DIAG/baked-pivot/PHASE_E_READY.md`. |
