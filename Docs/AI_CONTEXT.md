...
controller/clips, `PfGolfer_Olivia` at scale 1 with the FBX instance at Remy's 180° yaw), then the unchanged tool
chain. The contact radius is measured on the character's mesh at capture (Olivia 8.14 mm → `ContactM` 21.71; Remy
keeps 7.18). Steps 1–2 green (26/26 tests, wrap tips within 1.2 mm). Step 3: Cesar rejected the first pass at sight
(she faced away from the ball — my builder had left the instance at identity; a false "hand twist" rig diagnosis
...
---
## 2026-09-06 — `design_consistency_audit` — **DONE** (5 red-team rounds)

Spec: `Docs/Specs/Completed/design_consistency_audit/` (`DONE`, Cesar-approved). Notion 2112.
**The fix list in report § 5 is the Architect's input — the fixes themselves were never part of this
task.**
**Audit only — no production change.** Deliverable: `Docs/Reports/DESIGN_CONSISTENCY_AUDIT.md`.

> **Every number below is generated by `python3 Docs/Scripts/audit_numbers.py`, not typed.** If a
> figure anywhere disagrees with that script's output, the script is right. Corpus rule: **17
> distinct EN screens**, Inventory's four tab states collapsed to one, `MODAL_*` / `PREFAB*` /
> tripwire dumps excluded. Quoting a number without its corpus is what failed this task twice.

**Four findings the spec did not anticipate.**

**(1) Japanese renders on a Latin font asset — and the render is FINE.** With `CurrentLanguage =
Japanese`, **660 CJK labels are bound to a Latin font asset and 7 to NotoSansJP.** `LocalizedText`
swaps the STRING and never the font. **This is S3, not S1:** TMP's fallback chain renders CJK
correctly, and the red-team refuted the S1 claim by looking at the frame — the original finding was
graded on `font.name` alone with no visual evidence. The risk is metric/weight drift, not tofu.
...
`reference/NODE_RESOLUTION.md`; every id confirmed from the render call's own `original_width/height`.
...
(36 in-screen + 5 in `CharacterThumbnailCard`/`StatBar`). All are stat readouts, one coherent fix.

**(4) Auto-sized labels make the serialized `fontSize` fiction.** 35 of HomeScreen's 131 labels
auto-size; the tripwire moved one 49.05 → 51 from a font swap alone. **RENDERED px decides, never
the serialized number** — `fontSize × lossyScale.y ÷ canvas.scaleFactor`, stated in every JSON
header. Over n=2057 non-autosized labels: **1389 on-scale, 209 ÷1.4, 139 ÷1.2, 46 59/66, 274
unexplained.** The ÷1.4 bucket is the real one — the task began looking for a ÷1.2 era and the
corpus corrected it.

**Also measured:** `Outline`-as-border **20** (one ModeCard family); `Shadow`-as-drop-shadow **0 —
shape refuted**; `Image.Type.Filled` on **225** bar images (`Bar` 182 / `BarContainer` 33 /
`BarPending` 8 / `GhostBar` 2); 9-slice collapse is the ONLY FAIL rule in the whole 74-prefab sweep
(12 prefab + 30 live — the same oval-pill defect Cesar caught by eye on `stamina_boost_shop`);
**701 visible null-sprite fills, 291 of them panel-sized** (rule: `sprite == "<NONE>"` AND α > 0.2;
panel adds ≥200×60). The panel figure is stable across α 0.15–0.30; the *visible* figure is soft
(690–709) and the report says so.

**Instruments (Editor-only, `Assets/Editor/UIFidelity/`):** `DesignAuditDumper` (writes
`<Screen>__<locale>.json` — the locale suffix is load-bearing, see below), `DesignAuditRunner`
(real-navigation driver: tripwire / dump / deep / modals / capture / lint, armed across the domain
reload via `SessionState` + `[InitializeOnLoad]`), and a `LintRoot` overload proven **byte-identical**
to `LintPrefab`. `DesignAuditToolingTests` 12 tests; full EditMode sweep **2706 passed / 0 failed**.

**Five scars worth carrying forward — the audit took five red-team rounds, and four of them
failed on the same shape: a top-line corrected while a sub-table, header or fix-row kept an older
number.**

- **EN and JA passes wrote the SAME filename**, so each overwrote the other and the corpus held a
  mix of locales. Locale-INVARIANT properties then differed by locale — impossible, and that
  impossibility is how the red-team caught it. Fixed at the source, not in the numbers.
- **`tests-run`'s `Results` array lists only skipped/failed tests**, so a green summary is NOT
  evidence a new suite ran. Prove a new suite with a deliberate tripwire (here: 20 → 999).
- **FOUR successive checkers reported "0 stale" while real defects stood.** Each was written to
  confirm, never proven able to fail. A checker is not evidence until you have watched it fail on a
  defect you planted. Two of the blind spots are worth naming because they recur: one required the
  number to be its own bold span (`**442**`) and so could not see `**S2/S3 — 442 visible…**`; another
  parsed 4 of 5 table rows because a malformed first column dodged its regex, summed what it could
  see, and passed while a human read a different total. The current gate matches on paragraphs,
  checks breakdown sums, and **verifies every named object actually occurs in the corpus** — a
  fabricated `GhostBar / Fill | 3` row matching ZERO data shipped past three gates before that
  existed. Six planted defects, six caught.
- **A pass that dumps `CurrentScreenRoot()` must verify the re-seat landed.** The Tier-2 pass called
  `Force(id)`, got `true`, the screen never changed, and it dumped the still-active HomeScreen over
  HomeScreen's own file with a Tier-2 provenance. Measurements were identical, so no count moved and
  four gates passed clean — only the provenance was corrupted. Non-corpus dumps now carry a prefix
  (`MODAL_`, `TIER2_`) so they can neither collide with a corpus filename nor enter the corpus.
- **This corpus reproduces in its MEASUREMENTS, not its BYTES.** Countdown labels tick, an animating
  glow alpha samples at whatever phase the frame caught, `SettingsOverlay` carries the build number.
  Compare two corpora by measurement; an md5 diff of this evidence is always noise.

**Still open, listed in report § 6:** A5 node specs beyond the one pulled node, crop sheets for the
modals, the live-radius half of shape (ix), and 7 Tier-2 screens. Nine of the 17 surfaces were
re-seated via `ShowScreen` rather than tapped (no player path exists from a fresh session); each
such dump records `reachedVia` instead of passing itself off as a tap.
