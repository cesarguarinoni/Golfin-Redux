...
| Shot Controls | 🔶 **Phase 7 COMPLETE (83/83 tests)**. **Phase 8 Shot UI Polish IN PROGRESS** — Parts 8.1 + 8.2 + 8.2.5 + 8.3 complete (2026-04-28). **8.3 REDO DONE**: PlayerContext static bus + PlayerContextPopulator (Assembly-CSharp side), PlayerCardWidget reads from PlayerContext, HoleCardWidget has _defaultHoleMap fallback, all 18 hole maps fixed to Sprite, scene rebuilt from scratch with correct Figma layout (settings on own row at Y=24, cards at Y=158, portrait 180×180, chips 298×48 flat navy, Rubik 23px right-aligned). Play-mode screenshot confirms: portrait visible, hole map visible, text readable, no white boxes. Settings is white square (not circle) — knob sprite unavailable; flagged as polish. **Awaiting Architect ack before Part 8.4 (wind + hole indicators).** |
