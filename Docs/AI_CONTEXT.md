...
| Shot Controls | 🔶 **Phase 7 COMPLETE (83/83 tests)**. **Phase 8 Shot UI Polish IN PROGRESS** — Parts 8.1 + 8.2 + 8.2.5 + 8.3 + 8.4 + **8.5 (action button cluster) IN PROGRESS (2026-04-29)**. 2×2 SPIN/FADE-DRAW/GOLFIN/DRIVER cluster built in `LabScaffold.unity` via `ActionButtonsBuilder`. SpinPanel opens/closes correctly (cone hides, runtime Button closes on outside-tap). **Remaining for 8.5:** ball selector + club selector outside-tap close (both selectors broken — fix tomorrow). |
