...
**Last Updated:** 2026-05-05 (Architect: SPEC.md for `controls_c_fix` Phase A written and folder moved Queued → Active. STATUS=SPEC_READY, Tier 3 pipeline. Phase A scope: stop-check tolerance-window fix in both RunRollPhase + RunPuttPhase; narrow CSV tuning (putt.csv Green 0.10→0.50 / GreenCollar 0.14→0.40; surfaces.csv CartPath 0.06→0.30 — NO other surface); 5 new EditMode tests in new RollAndPuttTuningTests.cs. Bit-exact gate 198 → 203. Notion C.1+C.2 entry flipped to In Progress; new Phase B Notion entry `35631e0e-9a36-8102-b217-d00dac3c3d92` (Fairway/Rough/etc tuning) created and Queued. Implementer kickoff next.)
