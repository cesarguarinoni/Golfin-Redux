...
**Last Updated:** 2026-05-06 (`controls_f_drag_calibration_audit` **DONE end-to-end** — pipeline ran clean: implementer → reviewer (PASS-with-caveat since Unity MCP unavailable in their session) → Cesar manually verified `Window > Test Runner > EditMode > Run All`, **211 PASS / 0 IGNORED** confirmed → approved. Folder moved to `Docs/Specs/Completed/`. Notion `35731e0e-9a36-818d-9a4c-ee8dd9ca511c` flipped Done, Closed=2026-05-06. Final state: drag overlay multipliers 0.920/0.890/0.880 at v=60/70/80 m/s; smoothstep blend v∈[45,55]; driver carry 240→**249yd** (-9.5% vs Trackman 275, inside ±10%); irons unchanged (5-iron grazes seam, 0.6% within tolerance). CALIBRATION_METHODOLOGY.md §9 added (drag overlay) mirroring §3 (lift); §8 closed with cross-reference. **🎉 C-CLUSTER PHYSICS WORK COMPLETE.** Roadmap §1 (Putter P1) closing follow-ups all delivered: `controls_c_diagnosis` → `controls_c_fix` → `controls_d_velocity_cap_diagnosis` → `controls_e_aero_overlay_pass` → `controls_f_drag_calibration_audit`. Two-layer physics architecture (Layer 1 real-physics / Layer 2 corner-case overlay) is established and documented. **Next:** Loop v1 §2a (Ball state machine) is the next umbrella. Per Cesar's plan, scoping starts tomorrow.)
