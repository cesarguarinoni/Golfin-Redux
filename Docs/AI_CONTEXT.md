...
| Physics Architecture | 🔶 **ARCHITECTURAL PIVOT IN PROGRESS (2026-04-25)** — sim moving to baked JSON+heightmap data path; scene providers demoted to editor helpers. Tactical fix abandoned after three failed diagnoses (synthetic-test theatre → marker bug fix held but → high-velocity launch bug, none of which fixed Cesar's actual repro "ball instantly falls through green/bunker into void"). Active spec: `Docs/Specs/Active/SIM_BAKED_DATA_PATH.md`. Branch: `sim-baked-data-path`. Pre-pivot tag: `pre-baked-pivot`. Phases 0–6 of physics stable on `main`; pivot work happens entirely on the branch. B1/B2/B3 marker fixes (2026-04-25 morning) are KEPT — Course.SurfaceMarker components remain authoritative source for the new BakeZoneJsonTool. |
