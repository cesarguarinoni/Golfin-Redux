...
**Last Updated:** 2026-05-05 (Architect: SPEC.md for `controls_c_fix` Phase A written + hardened by adversarial review with web sources. STATUS=SPEC_READY, Tier 3 pipeline. Phase A scope: stop-check tolerance-window fix in both RunRollPhase + RunPuttPhase with **epsilon = stopSpeed²·0.05** [bumped from 0.01 after adversarial review showed 0.01 ≈ 1 LSB at Green stopThresh providing no meaningful tolerance over fp noise; 0.05 sized to absorb fp16.16 noise while preserving uphill safety]; narrow CSV tuning (putt.csv Green 0.10→0.50 / GreenCollar 0.14→0.40; surfaces.csv CartPath 0.06→0.30 — NO other surface); 5 new EditMode tests in new RollAndPuttTuningTests.cs. Bit-exact gate 198 → 203. SPEC also gained: corrected C.2 root-cause narrative (fp-rounding + sub-LSB resistance, NOT slope re-acceleration as originally proposed in NOTES.md); new "Model choice & literature alignment" section citing Kolkowitz/Holmes/Werner&Greig/Roh/Penner/Brown&McPhee; cited USGA Stimpmeter source for k=0.50; honest framing of CartPath as playability-calibrated. Notion C.1+C.2 entry flipped to In Progress; new Phase B Notion entry `35631e0e-9a36-8102-b217-d00dac3c3d92` (Fairway/Rough/etc tuning) created and Queued. Implementer kickoff next.)
