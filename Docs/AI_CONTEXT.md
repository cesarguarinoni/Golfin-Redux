...
| Shot Controls | 🔶 **Phase 7 COMPLETE (83/83 tests)**. **Phase 8 Shot UI Polish IN PROGRESS** — Parts 8.1 + 8.2 + 8.2.5 + 8.3 complete (2026-04-28). 8.3: PlayerCardWidget + HoleCardWidget + SettingsButton in LabScaffold. HoleContext static bus populates from PhysicsLabController.OnHoleLoaded via reflection on HoleMetadata. asmdef: `autoReferenced:true`, no Assembly-CSharp ref (build-order circular). Layout matches reference. **Awaiting Architect ack before Part 8.4 (wind + hole indicators).** |
