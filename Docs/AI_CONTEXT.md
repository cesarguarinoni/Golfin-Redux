...
**Last Updated:** 2026-05-05 (Architect: `controls_d_velocity_cap_diagnosis` Phase A SPEC.md written and locked. Adversarial review confirmed: the "64 m/s velocity cap" surfaced in `controls_c_diagnosis` is NOT a velocity cap — it's a Newton-Raphson convergence bug in `fpMath.Sqrt`. The early-exit clause `if (r >= prev) break` fires on iteration 0 because Newton always steps UP from a power-of-2 undershoot, returning the initial guess (which is `2^k` where k=floor(log2(input_raw)/2)). Cap value 64 = 2⁶ = 2²² / 2¹⁶ for driver dot-product ~10672. Same mechanism explains the 2.0 m/s putter quantization. Decision locked (Path B-narrow, libfixmath digit-by-digit port, single-pass int64): replace the entire `fpMath.Sqrt` body with the canonical Wikipedia / libfixmath shift-and-subtract algorithm — deterministic by construction, preserves project's integer-only Q16.16 determinism contract. Phase B follow-up created (`35731e0e-9a36-8132-96e4-cc27c4d2a734`) for Cos/Sin Taylor accuracy at angles near ±π (separate ~12% bug also visible in same captures); lands after Loop v1. Bit-exact 203/203 EditMode gate WILL break when Phase A lands; spec includes re-snapshot protocol. **Next:** fire the implementer subagent on `controls_d_velocity_cap_diagnosis` to repair `fpMath.Sqrt`, OR start scoping Loop v1 §2a (Ball state machine) in parallel — Phase A is NOT on Loop v1's critical path despite touching core physics.)
