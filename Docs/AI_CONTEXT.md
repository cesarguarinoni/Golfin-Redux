...
| Shot Controls | 🔶 **Phase 7 COMPLETE (83/83 tests)**. **Phase 8 Shot UI Polish IN PROGRESS** — Parts 8.1 + 8.2 + 8.2.5 + 8.3 + **8.4 (wind + hole indicators) COMPLETE (2026-04-29)**. WindIndicator + HoleIndicator built via `IndicatorWidgetBuilder`, wired to `WindIndicatorWidget` + `HoleIndicatorWidget`. HoleIndicator: perimeter-tracking tail (10px inset, 360°), chip slides to follow pin on X with wind-indicator gap clamp, CanvasGroup fade triggered by camera-forward dot product (fully gone by 90° off forward, tunable via `_fadeStartDot` inspector field). Pipeline lessons documented in `Docs/Diagnostics/PIPELINE_LESSONS.md`. |
