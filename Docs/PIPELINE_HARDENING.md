...
## 15. Clone-provenance YAML verifier — P1 (Order-611, 2026-07-06; hardened iter-2 2026-07-06)
...
- **The gate (HOOK-ENFORCED — `enforce_implementer_done.py` `validate_clone_provenance_yaml`):** for each element in `reuse_map.json` (the SPEC-side ground truth written by the implementer at start-of-task, not the prose table): verify via YAML that the built prefab contains a `PrefabInstance` block (`--- !u!1001`) whose `m_SourcePrefab` guid matches the cited source. CopyAsset clones (no `!u!1001` blocks) require explicit `!u!1001` lineage to be safe; sprite-guid equality alone is **explicitly rejected as proof** (see below).
  ...
  - **No PrefabInstance lineage + source sprite guid matches built sprite guid = CRITICAL FAIL** — a GUID string is copyable text. Pasting `m_Sprite: {guid: X}` from the source YAML does not prove the element was cloned via `AssetDatabase.CopyAsset`. SPEC §1.1: "Deliberately NOT checked: sprite equality as such." Sprite-guid equality is at most corroborating, never sufficient without `!u!1001` lineage. **(A1-mutant bypass, iter-2 red-team finding; closed in iter-2.)**
  - **Different real sprite + no PrefabInstance lineage = WARN** (legal re-skin A3 — not blocking; reviewer confirms). This preserves the A3 case: a CopyAsset re-skin that changes the sprite is valid.
  - Source has no sprite + built has a real sprite + no lineage = WARN (reviewer must confirm mapping is intentional; may be a name-mismatch in reuse_map).
- **Deliverable:** implementers on reuse-mandate tasks must author `Docs/Specs/Active/<task>/reuse_map.json` (see `Docs/Specs/TEMPLATE_reuse_map.json`) at start-of-task. To pass P1, all clones must use `PrefabInstance` (variant/nested prefab) workflow, not raw CopyAsset + sprite-paste.
- **Tests:** `TestCloneProvenanceYAML` in `test_enforce_implementer_done.py` — A1 (null-sprite fabrication), A1-mutant (guid-paste bypass), A2-A5g — 13 tests total. The A1-mutant test was added in iter-2 after the red-team demonstrated the bypass; its absence is the reason 107 green tests did not catch the vulnerability in iter-1.
