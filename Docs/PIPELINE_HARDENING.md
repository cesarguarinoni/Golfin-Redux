...
## 15. Clone-provenance YAML verifier — P1 (Order-611, iter-4 2026-07-06)

> **iter-4 (main-thread, Cesar-directed):** iter-3's live-editor scripts were DEAD — they used `class StructureCompare`/`class LintRerun` + `Debug.Log`, but `script-execute` requires `class Script` + `public static string Main()` and returns only the method's RETURN VALUE. Both seams always returned `None` → P1 blocked legit clones, P2 trusted the cited JSON. 113 green tests all monkeypatched the seam, so none drove the real RPC (parallel reviewer + red-team caught it end-to-end). iter-4 fixes the class/return, rewrites the compare to a **recursive skeleton signature EXCLUDING the root name** (a clone is renamed; verified: a *modified* clone matches modulo root name, a from-scratch forgery does not), makes P2 fail-CLOSED, and adds a **non-mocked `TestLiveEditorIntegration`** that drives the real editor (skips in CI). Proven E2E via the production verifier + live editor: real clone → PASS, guid-paste forgery → CRITICAL FAIL. **Nuance:** structural discrimination scales with substructure — cite composite Image elements / the prefab root, not bare leaf Images (leaves are backstopped by P2's null-sprite lint).
