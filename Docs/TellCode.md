...
- **[2026-05-08 14:00 JST → 2026-05-12 picked up] Ball visual rotation — SPEC moved to Active.** `BallAnimator.Update()` writes only `transform.position`, never `transform.rotation`. Spec at `Docs/Specs/Active/controls_i_ball_visual_rotation/SPEC.md` (STATUS=`SPEC_READY` 2026-05-12). Four open NOTES.md questions all locked by architect: (1) no rotation while Aiming, (2) reset rotation on SpawnInstance, (3) ball prefab confirmed has visible logo + dimple normal map (rotation will be visible), (4) `InternalsVisibleTo` already wired in `AssemblyInfo.cs`. Option A only — Option C deferred to `controls_j_ball_physics_rotation`. Picked up concurrent with Mac's §2d Results screen work; not on Loop v1 critical path. Notion `35a31e0e-9a36-81c0-9fc7-ea47902ef700` — flip to **In Progress** when implementer kicks off.
