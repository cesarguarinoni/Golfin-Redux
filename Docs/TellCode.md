...
- **[2026-05-08 14:00 JST] Ball visual rotation — ball slides instead of rolling.** `BallAnimator.Update()` writes only `transform.position`, never `transform.rotation`. Ball is a sphere SLIDING through world rather than rolling/spinning. NOTES at `Docs/Specs/Queued/controls_i_ball_visual_rotation/NOTES.md`, Notion `35a31e0e-9a36-81c0-9fc7-ea47902ef700` (Phase 10 Polish, P2, Order 260, Deferred). Cesar locked Option A (frame-to-frame position-delta rotation, ~12 lines, ~2-3 µs/frame mobile cost — negligible) for first ship; Option C (full physics-derived per-sample quaternion in Trajectory.Sample) deferred to follow-up `controls_j_ball_physics_rotation`. P2 polish, not Loop v1 critical path. Companion to OBFreeze framing flag below — both visual polish bucket.
