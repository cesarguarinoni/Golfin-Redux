...
- **[2026-05-08 14:00 JST → 2026-05-12 picked up]** ~~Ball visual rotation — SPEC moved to Active.~~ **CLOSED 2026-05-12 JST by `controls_i_ball_visual_rotation`.** Spec archived at `Docs/Specs/Completed/controls_i_ball_visual_rotation/`. `BallAnimator.Update()` now derives rotation from frame-to-frame position delta (Option A). Tests: `Passed=2, Failed=0`. Cesar live-play confirmed ball visibly rolls in direction of motion. Implementer silently corrected an architect-side cross-product sign error — lesson logged as **Lesson P** in `Docs/Diagnostics/PIPELINE_LESSONS.md` (sign-of-rotation-axis must be asserted with signed equality, not `Mathf.Abs`). One leftover cleanup: `Assets/Scripts/Physics/Tests/Editor/BallAnimatorTestAutoRunner.cs` joins the `Iter#TestRunner` housekeeping queue. Option C (`controls_j_ball_physics_rotation`) remains deferred.
