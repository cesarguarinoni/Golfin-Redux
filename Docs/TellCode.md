...
- `app_identity` (K15): ProjectSettings.asset + one-shot PlayerSettings icon call — NO scene; parallel-safe with everything open. Small git-conflict window with `build_stamp_hardening` (K3) (both write ProjectSettings.asset, different fields) — don't run the two in the same instant; either order works. Sim-valid (springboard/metadata class); one device look for the final call.
...
### K15 · app_identity — Surgical · sim-valid + one device look

```
Task: app_identity — the game's name is "Golfin: The Invitational", not
RE2, and the app icon is the one in Assets/Icons/. (Cesar, 2026-08-05.)

GROUNDED CURRENT STATE:
– ProjectSettings.asset line 16: productName: RE2 (line 15: companyName:
  DefaultCompany). Unity uses productName as the iOS CFBundleDisplayName
  and as the built .app name.
– Icon asset EXISTS and is store-grade: Assets/Icons/Golfin-Icon.png,
  1024×1024, NO alpha (App Store requires no-alpha — already compliant),
  GUID da98797204fa047d09ee14038b3f0ba2, imported as Sprite (textureType 8
  — acceptable for the icon slot; change the import only if the editor
  complains, and say so).

IMPLEMENTATION:
1. productName: RE2 → Golfin: The Invitational
   (quote it in the YAML — it contains a colon:
   productName: "Golfin: The Invitational").
2. companyName: DefaultCompany → NEXT INNOVATION PTE. LTD. (matches the
   signing team; cosmetic metadata — include unless Cesar objects).
3. Icon: assign Golfin-Icon.png as the DEFAULT icon (all platforms) —
   one-shot editor call, e.g.
     PlayerSettings.SetIcons(NamedBuildTarget.Unknown, new[]{tex},
                             IconKind.Any);
   via a temporary menu item (fine through MCP — instant call; the
   never-BuildPlayer-via-script-execute rule is about builds, not this).
   Per-platform iOS overrides NOT needed — Unity generates all iOS sizes
   from the default icon.
4. DO NOT TOUCH: applicationIdentifier (com.nextinnovation.golfingame),
   signing, provisioning — SOLVED, standing do-not-re-litigate rule.
   Product/display name changes do not affect signing.

KNOCK-ONS — handle in the SAME pass; they are why this isn't a 2-line task:
A. THE BUILT .APP RENAMES. The sim pipeline's tier-1 data swap targets the
   preserved Builds/SimApp/RE2.app, and IOS_SIMULATOR_LOOP.md +
   AI_CONTEXT.md reference RE2.app by name. After the rename:
   – report the exact new .app filename (Unity sanitizes the colon —
     confirm what it actually produces);
   – refresh the preserved sim app (one tier-2 append re-export; verify
     the incremental xcodebuild still works WITHOUT wiping the seeded
     DerivedData — expected yes; if not, STOP before any wipe and report);
   – update the RE2.app references in IOS_SIMULATOR_LOOP.md (AI_CONTEXT
     gets swept in the next context refresh — note it in the report).
B. HOME-SCREEN TRUNCATION: "Golfin: The Invitational" is 24 chars; iOS
   shows ~14–15 under the icon (→ "Golfin: The I…"). Implement as asked,
   REPORT a screenshot of how it renders; if Cesar dislikes the
   truncation, the dial is a short display name ("Golfin") with the full
   name kept for the store listing — his call, do not pre-decide.
C. ProjectSettings.asset is also written by the build stamp (buildNumber
   fields) and `build_stamp_hardening` (K3) edits its generator —
   different fields, no logic conflict, but avoid running this in the
   same instant as a build or the K3 pass (git-conflict window only).
D. ⚠️ Known adjacent landmine, NOT yours: ShellScene has unreconciled
   STALE MatchMakingModal prefab overrides that reappear on any ShellScene
   save (see the loading_bar_inset (K14) close-out). app_identity touches
   NO scene — keep it that way.

VERIFY — sim-valid (springboard/metadata class) + one device look:
1. Sim: app installs with the new icon + name on the springboard; report
   the truncation screenshot (knock-on B).
2. Device: one launch — icon + name on the home screen, game boots
   normally (Cesar's eyeball, final call).
3. git diff of ProjectSettings.asset = intended lines only (productName,
   companyName, icon block). Build stamp still renders.
4. Sim loop tier-2 incremental verified alive post-rename (knock-on A).
```
