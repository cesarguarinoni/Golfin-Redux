...
- **Shipped since (do NOT re-report as pending):** `mode_select_system` (341), `practice_1v1_matchmaking_split` (342), `tap_feedback_fx`, **`1v1_ingame_ui` Phase 1** (343, `756ab280`), **`1v1_match_flow` Phase 2a** (344, `ec9ee885`), and **`versus_bot_hardening`** (Order 345, commit `4e700ae5`, closed 2026-06-11 — H1 calibrated club/power via `bot_clubs.csv` carry table; H2 water layup/retarget via `Classify` + reactive `OBReason` bias (world-bounds OB NOT detectable proactively — Classify returns Fairway outside polygons); H3 green-slope read via additive `PutterGreenReader.TryGetSlopeAt` + CSV slope gain). All Completed.
- **NEXT / open:** **`versus_bot_difficulty`** (1v1 Phase 2b, Order 346) — SPEC WRITTEN 2026-06-11, SPEC_READY in `Docs/Specs/Active/versus_bot_difficulty/SPEC.md` (FULL PIPELINE). Design Cesar-approved: error injected POST-H2/H3 decision, pre-commit, NO safety re-check (intent vs execution — bot misses into water, never aims at it); club noise = ±1 band + power re-inverted to the same safe target via `InterpolateClubPower` (H2-safe by construction); club noise suppressed on putts; D2 error applies to putts after H3 slope read; bracket = highest `minLevel ≤ MatchContext.Players[1].Level`, resolved once per match; 6 brackets (minLevel 1/10/25/50/100/180) in new `bot_difficulty.csv`; `DebugLevelOverride` field for the bracket-1 vs bracket-6 same-hole video gate. Awaiting Cesar kickoff `Use the implementer subagent on "versus_bot_difficulty"`. After 2b: **2c** 1v1 result modal (deferred from 2a).
...
- **Last updated:** 2026-06-11 (Architect — wrote `versus_bot_difficulty` SPEC (Order 346, 2b difficulty model), SPEC_READY in Active/. Awaiting kickoff.)
