...
- **NEXT / open:** **1v1 Phase 2b** — the bot difficulty model (char-level → CSV error bands), now unblocked on the hardened baseline. Spec NOT yet written; Architect authors next. Locked design (from 2a decisions): difficulty derives from the opponent character's level via a CSV table (level/bracket → `{aimErrorDegMax, powerErrorMax, clubNoiseChance}`), randomized within the band per shot, higher level = tighter; card already shows the real level (Phase 1). Error injection layers ONTO the hardened bot (perturb post-safe-target aim/power, never override the H2 layup decision — settle this in the spec). Reference: `Docs/Specs/Completed/1v1_match_flow/SPEC.md` §13 + `Docs/Specs/Completed/versus_bot_hardening/` (IMPLEMENTER_REPORT has the H1 carry table + H2/H3 behavior details). After 2b: **2c** 1v1 result modal (deferred from 2a).
...
- **Last updated:** 2026-06-11 (Architect — verified `versus_bot_hardening` shipped `4e700ae5` + closed; Notion 345→Done. NEXT = 1v1 Phase 2b difficulty spec.)
