...
- **NEXT / open:** **`tree_collisions`** (Order 348, P2, Gameplay Polish) — SPEC WRITTEN 2026-06-11, SPEC_READY in `Docs/Specs/Active/tree_collisions/SPEC.md` (FULL PIPELINE). Ball flies through trees; fix is a bake into the deterministic sim (ball is NOT a Unity rigidbody — prefab colliders irrelevant): per-prefab `tree_collision_profiles.csv` (trunk+canopy cylinders) + per-hole `tree_obstacles.csv` harvested from all 3 placement sources (TreePlacer terrain TreeInstances, `StandaloneTrees` GOs, `PaintedTrees`) → `TreeObstacleProvider` XZ grid → shared segment-vs-cylinder test in ALL sim phases (airborne/bounce/roll/putt). Trunk = hard reflect (Layer 1), canopy = one-time entry-crossing impulse `canopyHitDamping` (Layer 2, D3 REVISED 2026-06-12 after Cesar playtest — per-step damping caused slow-mo descent, rejected; see STATUS iter-6), auto re-bake on `Hole_NN_Geo` scene save via staleness hash, absent CSV = zero behavior change. Bot stays tree-blind — `tree_aware_bot` filed as Order 351 (P3, Phase 2, after 348 ships). Awaiting Cesar kickoff `Use the implementer subagent on "tree_collisions"`. 1v1 pillar remains closed (347 reward display deferred).
...
- **Last updated:** 2026-06-11 (Architect — wrote `tree_collisions` SPEC (Order 348) after tree-system investigation; `tree_aware_bot` filed as Order 351 Phase 2. Awaiting kickoff.)
