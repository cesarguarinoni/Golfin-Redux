...
- **[2026-05-08 14:00 JST → 2026-05-12 picked up]** ~~Ball visual rotation — SPEC moved to Active.~~ **CLOSED 2026-05-12 JST by `controls_i_ball_visual_rotation`.** Spec archived at `Docs/Specs/Completed/controls_i_ball_visual_rotation/`. `BallAnimator.Update()` now derives rotation from frame-to-frame position delta (Option A). Tests: `Passed=2, Failed=0`. Cesar live-play confirmed ball visibly rolls in direction of motion. Architect-side cross-product sign error in SPEC was caught by Cesar in live play (ball rolled backward) and fixed mid-iteration by Code on Cesar's instruction — lesson logged as **Lesson P** in `Docs/Diagnostics/PIPELINE_LESSONS.md` (rotation-axis tests must assert signed equality, not `Mathf.Abs`). One leftover cleanup: `Assets/Scripts/Physics/Tests/Editor/BallAnimatorTestAutoRunner.cs` joins the `Iter#TestRunner` housekeeping queue. Option C (full physics-derived per-sample quaternion in `Trajectory.Sample`) not committed — may revisit if backspin/sidespin visual gap becomes worth the ~4–6h effort, otherwise stays shelved.
