...
- `hole_scene_leftover` (K16): editor tooling only — parallel-safe with everything. Touches SmokeRunner2eMenu (+ a VersusHudCaptureMenu audit); LabScaffold is saved ONLY to strip the serialized host residue; hole scenes are NEVER saved. EDITOR-verifiable, no sim/device needed.
...
### K16 · hole_scene_leftover — Surgical (editor tooling) · EDITOR-verifiable

```
Task: hole_scene_leftover — Hole_06_Geo keeps reappearing in the editor
hierarchy "from time to time." Cesar report 2026-08-05. Root cause found
statically; the repo already contains a comment naming the exact leftover.

ROOT CAUSE (source-verified):
SmokeRunner2eMenu — "GOLFIN/Smoke/Capture 2e OB Drop + TURN (Hole_06)":
  OpenLabAndHole(): opens LabScaffold (Single) + Hole_06_Geo (ADDITIVE),
  in EDIT mode.
  AttachHost(): adds SmokeRunner2eHost to LabRoot — then
  SaveAndEnterPlayMode() SAVES LabScaffold (host now SERIALIZED into the
  scene) and enters play.
  OnPlayModeStateChanged(EnteredEditMode): disarms the host — but NEVER
  closes Hole_06_Geo and never restores the pre-run scene setup.
→ every 2e OB capture leaves LabScaffold+Hole_06_Geo as the edit
hierarchy. VersusHudCaptureMenu stages the same way for the Hole-6 water
scenarios (~lines 319–376). SmokeRunner2fMenu ALREADY self-cleans
defensively — its comment: "Close any hole scenes that are NOT Hole_01_Geo
(e.g. Hole_06_Geo left over from a prior §2e run). This prevents
ScanForLoadedHoleSceneAtStartup from wiring to the WRONG HOLE" — which is
why this matters beyond clutter: a leftover hole scene can wire the lab to
the wrong hole on the next play run that doesn't pre-clean.
SECOND RESIDUE, same family as the CentralBall lesson (editor tools
persisting state into scenes): the SmokeRunner2eHost component stays
SERIALIZED in LabScaffold.unity between runs (the NEXT run strips it, but
the scene carries tool residue in the meantime).
NOTE: SaveAndEnterPlayMode saves ONLY LabScaffold — hole scenes are never
written. Confirmed; keep it that way.

FIX — the exit hook already exists; make it restore what it changed:
1. In OpenLabAndHole (BEFORE any OpenScene): capture the current setup —
   EditorSceneManager.GetSceneManagerSetup() — persist as JSON via
   SessionState (paths + active flags) so it survives the domain reload
   into/out of play mode.
2. In the existing OnPlayModeStateChanged(EnteredEditMode) handler, after
   the disarm: close the staged hole scene WITHOUT saving
   (EditorSceneManager.CloseScene(s, true)); strip the SmokeRunner2eHost
   from LabRoot and save LabScaffold ONCE, clean (removing the residue is
   the only legitimate LabScaffold write); then
   RestoreSceneManagerSetup(saved) so the hierarchy returns to whatever
   was open before the menu ran.
   ALTERNATIVE (report if chosen): adopt LoopV2SmokeBotMenu's pattern —
   inject the host at EnteredPlayMode, never saved to disk — which removes
   the LabScaffold write entirely. In-repo gold standard; slightly bigger
   diff. Either is acceptable; say which shipped.
3. Re-register the handler via [InitializeOnLoadMethod] (LoopV2 precedent)
   so a domain reload mid-flow can't orphan the cleanup.
4. AUDIT the same staging pattern in VersusHudCaptureMenu (both staging
   sites) and apply the same restore. Leave SmokeRunner2fMenu's defensive
   pre-clean IN PLACE — defense in depth; it should simply find nothing.

CONSTRAINTS:
– NEVER save a Hole_XX_Geo scene (generated content, no merge driver).
– Do not change what the captures DO — staging during the run stays
  identical; this is exit-path-only.
– SessionState, not EditorPrefs (must not leak across projects/sessions).

VERIFY — EDITOR:
1. Open ShellScene. Run the 2e OB capture end-to-end. After play exits:
   hierarchy shows ShellScene again (NOT LabScaffold+Hole_06_Geo), captures
   were produced, git status is CLEAN (no LabScaffold, no Hole_06_Geo
   modifications).
2. Run it twice back-to-back — the second run stages fine from the
   restored setup.
3. Run the 2f capture — its "Closing stale hole scene" log line no longer
   fires.
4. VersusHud water scenario: same restore behavior after its run.
```
