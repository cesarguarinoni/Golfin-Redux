...
- **NEXT / open:** `1v1_match_flow` (Order 344, P2) — 1v1 **Phase 2**. SPEC WRITTEN 2026-06-09, in `Docs/Specs/Active/1v1_match_flow/SPEC.md` (SPEC_READY, FULL PIPELINE). Scoped **Phase 2a** = turn-flow state machine + win/tie/draw + persistent winner banner + basic runtime bot (no error injection) + RP grant on win via `GameSession.OnMatchComplete` event bridge. Six design calls settled with Cesar (one ball + two persisted lies; draw on tie; strict alternation P1-first; courtesy auto-resolves; camera follows active; difficulty from char level via CSV in 2b; win reward = `versus_1v1.rewards`=200). **Phase 2b** (level→error-band difficulty model) deferred, written after 2a ships (SPEC §13). Awaiting Cesar glance → kickoff `Use the implementer subagent on "1v1_match_flow"`. Architect does NOT fire the chain.
...
- **Last updated:** 2026-06-09 15:58 JST (Architect — settled Phase-2 design calls with Cesar; wrote `1v1_match_flow` Phase-2a SPEC, moved folder Queued→Active/.)
