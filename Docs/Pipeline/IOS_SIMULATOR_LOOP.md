...
**`Builds/SimApp/GolfinTheInvitational.app`** (outside DerivedData, gitignored). The `Data/level<N>` swap
tier below works against that copy even if DerivedData is gone.

> **Renamed 2026-08-05** (`app_identity`). Was `RE2.app` until `productName` became
> `Golfin: The Invitational`. Unity sanitises that to `PRODUCT_NAME_APP = GolfinTheInvitational`
> (colon and spaces stripped), so the bundle, the executable and the process name are all
> **`GolfinTheInvitational`**; only `CFBundleDisplayName` — the springboard label — keeps the
> punctuation. Bundle id `com.nextinnovation.golfingame` did NOT change.
...
Re-check with `grep -c lib_burst_generated …` (expect `0`) and re-run the tier-2 command. Because the
pbxproj is regenerated by every export, **this fix must be re-applied after each append re-export** —
check the count before building rather than waiting for the error.
...
- Bundle id `com.nextinnovation.golfingame`, process/executable `GolfinTheInvitational`
...
- Unity `Debug.Log` capture: relaunch with `xcrun simctl launch --console-pty --terminate-running-process`
  and grep the piped file — `log show --predicate 'process=="GolfinTheInvitational"'` returns nothing
